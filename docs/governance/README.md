...
│     └─ Checks against rules defined in docs/governance/rules.md
...
1. Read `docs/governance/rules.md` to understand what's required and what's forbidden.
2. Don't silently violate rules. If a rule would block your work, talk to it first (update the rule in rules.md + change-log.md, and update the code that enforces it).

**If you're an AI tool (Claude, etc.) editing this repo:**
1. Read `docs/governance/rules.md` before you start.
2. Assume all rules are real constraints, not suggestions.
3. If a rule needs to change, update `rules.md` AND `change-log.md` AND the code that enforces it, in the same commit.
4. If a rule is marked "TBD" or "Planned", ask Javier before enforcing it.

**If you're changing the rules:**
1. Update `docs/governance/rules.md` to document the change.
2. Update the enforcement code (usually `scripts/governance-check.py`).
3. Add an entry to `docs/governance/change-log.md` with today's date and a one-line summary.
